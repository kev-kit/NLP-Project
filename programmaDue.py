...
def LMI(sostantivo, lis_agg, corpusToken):
	lis_agg = list(set(lis_agg)) 												# devo fare il cast di list() sulla set() in quanto set(lista) restituisce oggetto di forma "set(lista con una sola occorrenza per elemento)"
		...
		if hasattr(nodo, 'label'):
				if nodo.label() == "GPE":		# estraggo tutte le GPE
					for i in nodo.leaves():
						NE += i[0] + " " 		# devo aggiungere " " per dividere i nomi propi composta da piu' parole (cosi facendo ho per ogni gpe uno spazio in fondo alla parola) 
					gpe_entity.append(NE)		# in gpe_entity non mi interessa aggiungere la label
#	IOB = nltk.chunk.tree2conllstr(tree_net)
#	print IOB	
...
def main(file1, file2):
	...
	lmi_sost_agg = sorted(lmi_sost_agg, key = lambda elem: elem[0])								# ordina alfabetico
